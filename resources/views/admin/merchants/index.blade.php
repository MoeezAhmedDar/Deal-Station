...
@section('content')
                ...
                <div class="card">
                    ...
                    <div class="card-body pt-0">
                        <div style="width: 100%; overflow-x:auto;">
                            <!--begin::Table-->
                            <table class="table table-bordered align-middle fs-6 gy-5" id="merchants_table">
                                <!--begin::Table head-->
                                <thead>
                                    <!--begin::Table row-->
                                    <tr class="text-start text-dark fw-bolder fs-7 text-uppercase gs-0">
                                        ...
                                        <th>{{ __('Gov ID') }}</th>
                                        <th>{{ __('Status') }}</th>
                                        <th style="width: 25% !important;"> {{ __('Action') }}</th>
                                    </tr>
                                    <!--end::Table row-->
                                </thead>
                                <!--end::Table head-->
    ...
    <script>
        ...
        function loadTableData(merchant_id) {
            merchantsTable = $('#merchants_table').DataTable({
                'ajax': {
                    headers: {
                        "X-CSRF-Token": "{{ csrf_token() }}",
                    },
                    url: "{{ route('fetch-merchants') }}",
                    type: "POST",
                    data: {
        ...
        function filterFunc() {
            merchantsTable.DataTable().destroy();
        ...
        function removeFunc(id) {
            if (id) {
                Swal.fire({
                    text: "{{ __('Are you sure you want to delete merchant ?') }}",
                    icon: "warning",
                    showCancelButton: !0,
                    buttonsStyling: !1,
                    ...
                    if (result.isConfirmed) {
                        var url = "{{ route('merchants.destroy', ': id ') }}";
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                if (response.data) {
                                    Swal.fire("Done!",
                                        "{{ __('Merchant has been deleted successfully') }}",
                                        "success");
                                } else {
                                    Swal.fire("Error Deleting!",
                                        "{{ __('This merchant cannot be deleted because it is linked to a branch') }}",
                                        "error");
                                }
                            },
